# LINEログ整理アプリ UIリデザイン Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** LINEログ整理アプリのStreamlit UIをティールカラーのダッシュボード型クリーンデザインにリデザインする。

**Architecture:** `line_log_app.py` 1ファイルのみ変更。ロジック部分（パース・フィルタ・削除処理）は一切変更せず、UI描画部分だけをHTMLコンポーネント＋カスタムCSSに置き換えるハイブリッド方式。`st.data_editor` の編集機能はそのまま維持する。

**Tech Stack:** Python, Streamlit, pandas, openpyxl, HTML/CSS (unsafe_allow_html)
...
| ファイル | 操作 | 内容 |
|---|---|---|
| `line_log_app.py` | 新規作成 | Streamlit アプリ本体（チャットのコードをベースにUIを改修） |

---

## Task 1: line_log_app.py を作成する（オリジナルコード）

> UIを変更する前のベースラインとして保存する。

**Files:**
- Create: `line_log_app.py`

- [ ] **Step 1: ファイルを作成する**

以下の内容でそのまま `line_log_app.py` を作成する（仕様書の会話でユーザーが貼り付けたコードと同一）:
...
```bash
streamlit run line_log_app.py
```

ブラウザで `http://localhost:8501` を開き、アプリが起動することを確認する。

- [ ] **Step 3: コミット**

```bash
git add line_log_app.py
git commit -m "feat: add line log app (base, pre-redesign)"
```

---

## Task 2: グローバルCSS注入ブロックを追加する

> Streamlit のデフォルトスタイルを上書きするCSSを1か所にまとめて注入する。

**Files:**
- Modify: `line_log_app.py`

- [ ] **Step 1: `set_page_config` の直後にCSS関数を追加する**

`st.set_page_config(...)` の行の直後、`st.title(...)` の前に以下を追記する:
...
- [ ] **Step 2: アプリ先頭で `_inject_css()` を呼び出す**

`st.set_page_config(...)` の直後（`st.title(...)` の前）に追加する:
...
ページ背景が `#f1f5f9`（薄いグレー）になっていることを確認する。

- [ ] **Step 4: コミット**

```bash
git add line_log_app.py
git commit -m "feat: inject global CSS for teal business theme"
```
...
> ヘッダー・KPIカード・セクションタイトル・情報バナーの描画関数を追加する。

**Files:**
- Modify: `line_log_app.py`

- [ ] **Step 1: `to_excel` 関数の直後に4つのヘルパー関数を追加する**
...
```bash
git add line_log_app.py
git commit -m "feat: add UI helper functions for header, KPI cards, banners"
```

---

## Task 4: ページヘッダーを置き換える

> `st.title()` と `st.write()` をカスタムHTMLヘッダーに差し替え、ファイル読み込み後は期間も表示する。

**Files:**
- Modify: `line_log_app.py`

- [ ] **Step 1: `_inject_css()` 呼び出しの直後の `st.title` と `st.write` を削除し、`render_header()` に置き換える**
...
- [ ] **Step 2: ファイル読み込み後のヘッダーを期間付きに更新する**

`if uploaded_file is not None:` ブロック内で `st.success(...)` の直前に以下を追加する（ファイル読み込み済みの場合にヘッダーを再描画してファイル名・期間を表示する）:
...
if uploaded_file is not None:
    ...
    st.success("ファイルを読み込みました。")
    # st.write(f"ファイル名：{uploaded_file.name}")  ← この行は削除（ヘッダーに移動）
```

`st.write(f"ファイル名：{uploaded_file.name}")` の行を削除する。
...
- ファイル未アップロード時: ティールのヘッダーが表示される
- ファイルアップロード後: ヘッダー右側にファイル名と対象期間が表示される

- [ ] **Step 4: コミット**

```bash
git add line_log_app.py
git commit -m "feat: replace title with teal gradient header component"
```

---

## Task 5: KPIカードと情報バナーを追加する

> ファイル読み込み後の数値を4枚のKPIカードに表示し、`st.info()` をティールバナーに置き換える。

**Files:**
- Modify: `line_log_app.py`

- [ ] **Step 1: `else:` ブロック（parse_failed でない場合）の先頭を置き換える**

`if st.session_state.get("parse_failed"):` ... `else:` ブロック内の以下の既存コードを:
...
ファイルアップロード後に4枚のKPIカードとティールの情報バナーが表示されることを確認する。

- [ ] **Step 3: コミット**

```bash
git add line_log_app.py
git commit -m "feat: add KPI cards and teal info banner"
```

---

## Task 6: セクションタイトルを置き換える

> `st.subheader()` を `section_title()` に差し替え、セクション区切りのデザインを統一する。

**Files:**
- Modify: `line_log_app.py`

- [ ] **Step 1: 「整理結果」のセクション先頭を置き換える**
...
# 変更後
section_title("整理結果", count=len(display_df) if not current_df.empty else 0)
```

`if not current_df.empty:` ブロック内で `st.subheader("整理結果")` の後に `categories = ...` / `selected_category = ...` のコードがあるため、順序に注意する。正しい順序は以下:
...
既存の `st.write("削除したいログは...")` と `st.write("進捗状況は...")` の2行は削除する。

- [ ] **Step 2: 「分類別件数」セクションを置き換える**
...
3つのセクションタイトルが横線付きの大文字スタイルで表示されることを確認する。

- [ ] **Step 5: コミット**

```bash
git add line_log_app.py
git commit -m "feat: replace subheaders with styled section titles"
```

---

## Task 7: 分類別件数をメトリクスカードに置き換える

> `st.dataframe(count_df)` を `st.columns` + `st.metric()` のカードグリッドに変更する。

**Files:**
- Modify: `line_log_app.py`

- [ ] **Step 1: 分類別件数の描画コードを置き換える**
...
分類別件数がカード形式で表示されることを確認する。

- [ ] **Step 3: コミット**

```bash
git add line_log_app.py
git commit -m "feat: replace category count dataframe with metric cards"
```

---

## Task 8: 全体動作確認と最終コミット

- [ ] **Step 1: `sample_log.txt` でE2Eテスト**
...
以下を順番に確認する:
1. ティールヘッダーが表示される
2. `sample_log.txt` をアップロードする
3. ヘッダーにファイル名と期間が表示される
4. KPIカード4枚が表示される
5. ティール情報バナーが表示される
6. 「整理結果」セクションタイトルが表示される
7. テーブルのフィルター・編集・削除チェックが正常動作する
8. 削除ボタンがレッドで表示され、クリックで削除結果一覧へ移動する
9. 分類別件数がメトリクスカードで表示される
10. ダウンロードボタンがティールグラデーションで表示される
11. Excelダウンロードが正常に動作する

- [ ] **Step 2: 最終コミット**

```bash
git add line_log_app.py
git commit -m "feat: complete UI redesign — teal dashboard theme for LINE log app"
```
